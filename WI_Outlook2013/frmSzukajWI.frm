...
Private Sub UserForm_Initialize()
    ' Odczyt p�l tekstowych z rejestru
    ...
    ' --- NOWO��: Checkboxy domy�lnie zaznaczone na True ---
...
Private Sub btnSzukaj_Click()
    ...
    ' 2. Pakowanie �cie�ek z listy do jednego ci�gu tekstowego
    Dim i As Integer
    ...
    ' --- NOWO��: Zapisywanie stanu checkbox�w ---
    ...
    ' 4. Przygotowanie kluczy dla modu�u g��wnego
...
' -------------------------------------------------------------------------
' POPRAWIONY KOD OBS�UGI LISTY �CIE�EK DLA OUTLOOKA
' -------------------------------------------------------------------------
...
Private Sub btnRemovePath_Click()
    Dim i As Integer
    ' P�tla od ty�u, �eby nie zgubi� indeks�w przy usuwaniu
...
' -------------------------------------------------------------------------
' FUNKCJA ANIMUJ�CA PASEK POST�PU
' -------------------------------------------------------------------------
Public Sub AktualizujPostep(procent As Single, tekst As String)
    On Error Resume Next
    
    ' Je�li formularz jest z jakiego� powodu ukryty, upewnij si�, �e jest widoczny bez blokowania kodu
    If Me.Visible = False Then Me.Show vbModeless
    
    ' Maksymalna szeroko�� to szeroko�� etykiety pe�ni�cej rol� t�a
    Dim maxWidth As Single
    maxWidth = Me.lblPrgBg.Width
    
    ' Zabezpieczenie przed warto�ciami z kosmosu
    If procent < 0 Then procent = 0
    If procent > 1 Then procent = 1
    
    ' Zmiana szeroko�ci kolorowego paska
    Me.lblPrgBar.Width = maxWidth * procent
    
    ' Aktualizacja tekstu (opcjonalnie)
    Me.lblPrgText.Caption = tekst & " (" & Format(procent, "0%") & ")"
    
    ' BARDZO WA�NE: Wymusza od�wie�enie grafiki okna. Bez tego pasek si� "zawiesi" do ko�ca p�tli.
    DoEvents
End Sub
